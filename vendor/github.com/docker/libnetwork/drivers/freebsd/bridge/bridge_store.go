...
func (d *driver) populateEndpoints() error {
	...
	for _, kvo := range kvol {
		...
		if !ok || len(ep.id) < 7 || len(ep.nid) < 7 {
			logrus.Debugf("Network (%s) not found for restored bridge endpoint (%s)", ep.nid, ep.id)
			logrus.Debugf("Deleting stale bridge endpoint (%s) from store", ep.nid)
			if err := d.storeDelete(ep); err != nil {
				logrus.Debugf("Failed to delete stale bridge endpoint (%s) from store", ep.nid)
			}
			continue
		}
