...
func getDefGWInterface() string {
	...
	messages, err := route.ParseRIB(route.RIBTypeRoute, rib)
		...
		for _, addr := range addresses {
			if linkAddr, ok = addr.(*route.LinkAddr); !ok {
				continue;
			}
			return linkAddr.Name
		}
        }
	return ""
}

func addPFNatRule(config *networkConfiguration, i *bridgeInterface) error {
	nid := config.ID
	bridgeIf := config.BridgeName
	extif := getDefGWInterface()

	fname := "/var/lib/docker/network/files/pf." + nid[0:7]
	...
	natrule := fmt.Sprintf("nat on %s from %s:network to any -> (%s)\n",extif, bridgeIf, extif)
	f.WriteString(natrule)
	f.Close()
	err = exec.Command("/sbin/pfctl", "-a", "docker/"+nid[0:7], "-f", fname).Run()
	if err != nil {
		fmt.Printf("here\n")
		return err
	}
